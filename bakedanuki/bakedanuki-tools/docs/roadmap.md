...
### bdChannelBoxの複数属性拡張（開発完了）

複数属性の選択と一括操作の要望に対応し、数値の直接入力と基本メニューを実装しました。
既存のViewを常時利用できる一覧へ移行し、自動テストとMaya 2025本体検証が完了しています。
後半のCopy / Paste、検索、Step保存、表示調整、数値欄の入力改善まで利用者が動作確認し、
2026-09-24に今回の開発を完了しました。結果は[検証記録](bd_channel_box.md#検証)を参照します。
...
- [x] 選択中でStep欄を持つ属性への同じ表示stepの入力と、対象ごとの増減方式の維持
...
- [x] Nice Name・属性名・正式pathの検索と、検索欄の3段階表示設定
- [x] 正式属性path・数値種別ごとのStep設定の永続化と、右クリックからの全属性／選択属性リセット
- [x] 正常なPasteでは通知を表示せず、開始できないclipboardエラーだけを表示
- [x] 選択色を属性名へ限定し、入力欄との余白・背景色・boolの未選択表示を調整
- [x] float値欄・Step欄・Slider付き値欄を初回クリックで全選択し、再クリックとドラッグ編集を維持
- [x] 後半のUX改善を含む一連の機能を利用者が確認し、今回の開発を完了（2026-09-24）
...
Slider操作範囲のカスタマイズも、用途が具体化したときに別途検討します。
...
キーフレーム対応は意図的に保留しています。別途構築中のキーフレーム系基盤を統合した後に、
bdChannelBoxから利用する公開APIと責務分担を確認してから再開します。それまでは、キー操作を
bdChannelBox内へ重複実装せず、キー付き・入力接続済み属性を表示専用とする現行仕様を維持します。
再開時は、現在時刻とAnimation Layer、既存接続、値編集とキー操作の区別、複数対象の失敗方針、
一回のUndo境界を先に定義します。
