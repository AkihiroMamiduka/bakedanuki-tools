...
利用者による確認はまだ行っていません。結果は[検証記録](bd_channel_box.md#検証)を参照します。
...
以下は初回実装を終えた複数属性拡張とは別の候補です。
将来の入力拡張を見据えて属性選択と一括書込みの経路を分けています。
今回の対象には含めず、操作の意味と対象型を決めてから着手します。

| 候補 | 着手を検討する状況 | 設計時に確認する点 |
| --- | --- | --- |
| 上下・Slider・bool・enumの複数属性入力 | 数値直接入力以外も選択属性へ適用したい | 絶対値／差分、各行のstep・範囲・単位、boolの反転、enum定義の互換性、連続操作のUndo境界を決める |
| 選択属性の値Copy/Pasteメニュー | ノード間で選択属性の値を保持・転送したい | コピー時点の値か参照か、同名対応か選択順対応か、型・単位・定義不一致の扱いを決める |
...
- 検証用Maya 2025の終了待ちタイムアウトは原因未特定。今回の本体検証は2回連続で正常終了したが、
  原因解決とは扱わず、再発時に操作結果とprocessの終了を区別して調査する。
