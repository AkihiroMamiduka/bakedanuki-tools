...
- 現在の選択リストの index 0 が基準です。基準名と選択数を画面上部へ表示します。
...
- 同じ正式属性pathと型・単位種別の属性へ絶対値を一括入力します。
  距離と角度は同名でも対応付けません。対応しないnodeや編集不可の理由は件数の
...
明示的な「揃える」ボタンだけが、基準の未丸め実値を他の編集可能な対象へ適用します。
...
属性の表示名・範囲設定を変更した後は「更新」でView構成を読み直せます。
