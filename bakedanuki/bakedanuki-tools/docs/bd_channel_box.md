...
2026-09-24に、複数属性編集、Copy / Paste、検索、Step保存、表示・入力のUX調整まで
...
- 属性名からのドラッグ、または数値欄からの縦ドラッグでも範囲を選べます。
  数値欄内の横ドラッグは文字選択として扱います。初回クリックの全選択後も、
  再クリックと横ドラッグで通常の文字編集へ移れます。
...
float値欄、Step欄、Slider付き値欄は、マウスで初回フォーカスした左クリックで
表示中の入力文字全体を選択します。そのまま数値を入力すると既存値を置換します。
フォーカス中の再クリックはカーソル移動、横ドラッグは任意範囲の文字選択として扱い、
上下ボタン、ホイール、右クリックの動作は変更しません。
...
変更したstepは正式属性pathと型区分（数値／距離／角度）ごとに保持します。
別ノードの同属性、選択解除・再選択、更新、Undo / Redo、scene切替でも維持し、
複数選択への入力時は対象となった各属性の設定を同時に更新します。その後は各行を
個別に変更できます。初期値と異なるstepは`bd_channel_box/preferences/main`へ保存し、
Windowの終了・reload・Maya再起動後も復元します。sceneとUndo履歴には保存しません。
...
### 数値欄の初回クリック全選択（2026-09-24）

float値欄、Step欄、Slider付き値欄で、マウス操作により初めてフォーカスした
左クリックだけ入力文字全体を選択するようにしました。フォーカス中の再クリックは
通常どおりカーソルを移動し、横ドラッグによる範囲選択、上下ボタン、ホイール、
右クリックの動作も維持します。utilの汎用APIは既定で無効とし、bdChannelBoxが
数値Viewを生成するときだけ有効にします。

| 確認対象 | 結果 |
| --- | --- |
| toolsのBlack・Pyright・unit test | 成功。unit 8件、Maya 2025 / 2026 / 2027のPyright errorは0件 |
| Maya 2025 / 2026のtools runtime test | 各182件成功 |
| Maya 2027のtools runtime test | OS clipboard依存の既存1件を除く181件成功 |
| utilの全体検証 | Blackと全versionのPyrightが成功。Maya 2025の全体testは4,268件成功・763件skip |
| utilのQt / Maya UI互換性 | OS clipboard依存testを除き、各versionでQt 863件・Maya UI 377件が成功 |
| 初回クリック | Float値・Step・Slider付き値欄で全選択し、フォーカス中の再クリックでは通常のカーソル配置へ戻ることを確認 |
| 横ドラッグ | 初回クリックの全選択を取り消し、ドラッグした文字範囲だけを選択することを確認 |
| Maya 2025本体の操作 | `result.json`の`success`はTrue。初回クリック全選択を含む46工程と36枚の画像を確認 |

Maya 2025本体では、値欄の初回クリックで全文字が選択され、同じ欄を再クリックすると
通常のカーソル配置へ戻ること、Step欄も初回クリックで全選択されること、既存の
複数属性選択が維持されることを確認しました。結果と画像は
`%TEMP%/bd-channel-box-maya2025-ra69w5ny`です。機能結果の保存後に既知の
Maya終了待ちタイムアウトが発生したため、runnerの終了codeは1でした。

標準のutil UI全体testでは、この実行環境でOS clipboardが空を返す既存testが
Maya 2025で3件、Maya 2026 / 2027で各2件失敗しました。今回追加したtestは全versionで
各111件成功しており、clipboard依存testを除いたQt/UI testは全versionで成功しています。
Maya 2026 / 2027本体の画面操作は実施していません。
