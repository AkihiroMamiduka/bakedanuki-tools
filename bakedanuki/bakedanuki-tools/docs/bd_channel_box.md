...
自動テスト・Maya 2025本体での操作検証が完了しました。利用者による確認はまだ行っていません。
...
選択した属性名と数値欄を強調表示します。属性を選ぶだけでは値・状態・Undoを変更しません。
...
現段階で複数属性へ適用する数値操作は、文字による直接入力と数値の貼付けです。
値欄の上下操作・Step・Slider・bool・enumの通常入力は、選択数にかかわらず操作した1行の
対応ノードへ適用します。これらを複数属性へ適用する操作は将来の拡張候補です。
...
`bd_channel_box/ui.py` は公開Windowと配置・reload、`widget.py` は属性行とメニュー、
`table.py` は属性選択と数値直接入力、`controller.py` は基準node・対応属性・
...
値をtable modelへ二重に保存しません。選択中の複数属性への数値入力だけ一時的な文字欄で受け付けます。
...
複数行の値操作はutilの`apply_plugs_values()`へ型付きの編集要求を渡し、
全件の事前検証・復旧・1回のUndoを委譲します。将来の入力操作も同じ書込み経路を利用できます。
