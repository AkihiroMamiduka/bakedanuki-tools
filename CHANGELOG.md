...
- bdChannelBox上部へ「設定」メニューを追加し、「未フォーカス時のホイール編集」で
  数値の値欄とStep欄のマウスオーバー操作をまとめてON／OFFできるようにする。
  初期値はONとし、OFFではフォーカス中だけ数値を変更して、未フォーカス時のホイールを
  一覧のスクロールへ渡す。通常行とSlider付き行の値欄へ適用し、行の再構築後も維持する。
  ...
  `reset_layout()`、scene、Undo履歴には影響しない。enumはこの変更の対象外。
