...
- bdChannelBoxの値編集モードへ、選択属性値のOSクリップボードCopyと、
  選択中の1個または複数nodeへの同一正式path Paste、コピーした一値を選択属性へ貼る操作を追加。
  Copyは基準nodeの未丸め値と型・単位・enum定義を保存し、Pasteは表示行や行順に依存せず、
  非表示属性も同じpathと型で照合する。欠落・型違い・enum定義違い・編集不可属性は
  対象外として理由を表示し、適用対象は全件事前検証・一回Undo・失敗時復旧で変更する。
  一値の選択属性Pasteは、数値・距離・角度・boolを同じ型区分、enumを同じ定義へだけ展開し、
  クリップボードに一属性値だけがある場合に有効になる。行順による対応付けは行わない。
